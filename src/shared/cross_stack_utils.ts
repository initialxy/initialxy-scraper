...
export function generateFFmpegCommand(url: string, headers: Record<string, string>): string {
  ...
  return `ffmpeg -allowed_extensions ALL -protocol_whitelist file,http,https,tcp,tls -extension_picky 0 -readrate 2 -reconnect 1 -reconnect_at_eof 1 -reconnect_streamed 1 -reconnect_delay_max 2000 -timeout 300000000 ${headersStr} -i "${url}" -acodec copy -bsf:a aac_adtstoasc -vcodec copy out.mp4`;
}
